...
print('[6;30;44m' + 'Opcode:bswap_r64' + '[0m')

R1 = BitVec('R1', 64)

PK_R1 = (Concat(Concat(Extract( R1.size() - 56 - 1, R1.size() - 64, R1  ) , Concat(Extract( R1.size() - 48 - 1, R1.size() - 56, R1  ) , Concat(Extract( R1.size() - 40 - 1, R1.size() - 48, R1  ) , Extract( R1.size() - 32 - 1, R1.size() - 40, R1  ) ))), Concat(Extract( R1.size() - 24 - 1, R1.size() - 32, R1  ) , Concat(Extract( R1.size() - 16 - 1, R1.size() - 24, R1  ) , Concat(Extract( R1.size() - 8 - 1, R1.size() - 16, R1  ) , Extract( R1.size() - 0 - 1, R1.size() - 8, R1  ) )))) )
PS_R1 = (Concat((Concat((Concat((Extract (7, 0, ((R1)))), (Extract (15, 8, ((R1)))))), (Concat((Extract (23, 16, ((R1)))), (Extract (31, 24, ((R1)))))))), (Concat((Concat((Extract (39, 32, ((R1)))), (Extract (47, 40, ((R1)))))), (Concat((Extract (55, 48, ((R1)))), (Extract (63, 56, ((R1))))))))))
