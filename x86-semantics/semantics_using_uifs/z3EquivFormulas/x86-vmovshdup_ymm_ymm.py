...
print('[6;30;44m' + 'Opcode:vmovshdup_ymm_ymm' + '[0m')

R1 = BitVec('R1', 256)
R2 = BitVec('R2', 256)

PK_R2 = (Concat(Concat(Concat(Extract( R1.size() - 0 - 1, R1.size() - 32, R1  ) , Extract( R1.size() - 0 - 1, R1.size() - 32, R1  ) ), Concat(Extract( R1.size() - 64 - 1, R1.size() - 96, R1  ) , Extract( R1.size() - 64 - 1, R1.size() - 96, R1  ) )), Concat(Concat(Extract( R1.size() - 128 - 1, R1.size() - 160, R1  ) , Extract( R1.size() - 128 - 1, R1.size() - 160, R1  ) ), Concat(Extract( R1.size() - 192 - 1, R1.size() - 224, R1  ) , Extract( R1.size() - 192 - 1, R1.size() - 224, R1  ) ))) )
PS_R2 = (Concat((Concat((Concat((Extract (255, 224, ((R1)))), (Extract (255, 224, ((R1)))))), (Concat((Extract (191, 160, ((R1)))), (Extract (191, 160, ((R1)))))))), (Concat((Concat((Extract (127, 96, ((R1)))), (Extract (127, 96, ((R1)))))), (Concat((Extract (63, 32, ((R1)))), (Extract (63, 32, ((R1))))))))))
