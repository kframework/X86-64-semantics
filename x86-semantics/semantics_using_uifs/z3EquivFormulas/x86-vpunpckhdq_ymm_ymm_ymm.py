...
print('[6;30;44m' + 'Opcode:vpunpckhdq_ymm_ymm_ymm' + '[0m')

R1 = BitVec('R1', 256)
R2 = BitVec('R2', 256)
R3 = BitVec('R3', 256)

PK_R3 = (Concat(Concat(Concat(Extract( R1.size() - 0 - 1, R1.size() - 32, R1  ) , Extract( R2.size() - 0 - 1, R2.size() - 32, R2  ) ), Concat(Extract( R1.size() - 32 - 1, R1.size() - 64, R1  ) , Extract( R2.size() - 32 - 1, R2.size() - 64, R2  ) )), Concat(Concat(Extract( R1.size() - 128 - 1, R1.size() - 160, R1  ) , Extract( R2.size() - 128 - 1, R2.size() - 160, R2  ) ), Concat(Extract( R1.size() - 160 - 1, R1.size() - 192, R1  ) , Extract( R2.size() - 160 - 1, R2.size() - 192, R2  ) ))) )
PS_R3 = (Concat((Extract (255, 224, ((R1)))), (Concat((Extract (255, 224, ((R2)))), (Concat((Extract (223, 192, ((R1)))), (Concat((Extract (223, 192, ((R2)))), (Concat((Extract (127, 96, ((R1)))), (Concat((Extract (127, 96, ((R2)))), (Concat((Extract (95, 64, ((R1)))), (Extract (95, 64, ((R2))))))))))))))))))
