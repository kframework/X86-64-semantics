...
print('[6;30;44m' + 'Opcode:vpmovzxwd_ymm_xmm' + '[0m')

R1 = BitVec('R1', 256)
R2 = BitVec('R2', 256)
CONST_BV_S16_V0 = BitVecVal(0, 16)

PK_R2 = (Concat(Concat(Concat(Concat((CONST_BV_S16_V0), Extract( R1.size() - 128 - 1, R1.size() - 144, R1  ) ), Concat((CONST_BV_S16_V0), Extract( R1.size() - 144 - 1, R1.size() - 160, R1  ) )), Concat(Concat((CONST_BV_S16_V0), Extract( R1.size() - 160 - 1, R1.size() - 176, R1  ) ), Concat((CONST_BV_S16_V0), Extract( R1.size() - 176 - 1, R1.size() - 192, R1  ) ))), Concat(Concat(Concat((CONST_BV_S16_V0), Extract( R1.size() - 192 - 1, R1.size() - 208, R1  ) ), Concat((CONST_BV_S16_V0), Extract( R1.size() - 208 - 1, R1.size() - 224, R1  ) )), Concat(Concat((CONST_BV_S16_V0), Extract( R1.size() - 224 - 1, R1.size() - 240, R1  ) ), Concat((CONST_BV_S16_V0), Extract( R1.size() - 240 - 1, R1.size() - 256, R1  ) )))) )
PS_R2 = (Concat((Concat((Concat((Concat((CONST_BV_S16_V0), (Extract (127, 112, ((R1)))))), (Concat((CONST_BV_S16_V0), (Extract (111, 96, ((R1)))))))), (Concat((Concat((CONST_BV_S16_V0), (Extract (95, 80, ((R1)))))), (Concat((CONST_BV_S16_V0), (Extract (79, 64, ((R1)))))))))), (Concat((Concat((Concat((CONST_BV_S16_V0), (Extract (63, 48, ((R1)))))), (Concat((CONST_BV_S16_V0), (Extract (47, 32, ((R1)))))))), (Concat((Concat((CONST_BV_S16_V0), (Extract (31, 16, ((R1)))))), (Concat((CONST_BV_S16_V0), (Extract (15, 0, ((R1))))))))))))
