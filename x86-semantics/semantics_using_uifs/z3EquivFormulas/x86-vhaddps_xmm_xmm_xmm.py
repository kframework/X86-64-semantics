...
print('[6;30;44m' + 'Opcode:vhaddps_xmm_xmm_xmm' + '[0m')
...
PK_R3 = (Concat((CONST_BV_S128_V0), Concat(Concat( add_single ( Extract( R1.size() - 160 - 1, R1.size() - 192, R1  ) , Extract( R1.size() - 128 - 1, R1.size() - 160, R1  )  ) ,  add_single ( Extract( R1.size() - 224 - 1, R1.size() - 256, R1  ) , Extract( R1.size() - 192 - 1, R1.size() - 224, R1  )  ) ), Concat( add_single ( Extract( R2.size() - 160 - 1, R2.size() - 192, R2  ) , Extract( R2.size() - 128 - 1, R2.size() - 160, R2  )  ) ,  add_single ( Extract( R2.size() - 224 - 1, R2.size() - 256, R2  ) , Extract( R2.size() - 192 - 1, R2.size() - 224, R2  )  ) ))) )
PS_R3 = (Concat((CONST_BV_S128_V0), (Concat((Concat((Concat(( add_single ( (Extract (95, 64, ((R1)))), (Extract (127, 96, ((R1)))))), ( add_single ( (Extract (31, 0, ((R1)))), (Extract (63, 32, ((R1)))))))), ( add_single ( (Extract (95, 64, ((R2)))), (Extract (127, 96, ((R2)))))))), ( add_single ( (Extract (31, 0, ((R2)))), (Extract (63, 32, ((R2))))))))))
