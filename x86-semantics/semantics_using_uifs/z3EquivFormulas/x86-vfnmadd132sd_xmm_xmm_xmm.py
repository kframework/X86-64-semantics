...
print('[6;30;44m' + 'Opcode:vfnmadd132sd_xmm_xmm_xmm' + '[0m')
...
PK_R3 = (Concat((CONST_BV_S128_V0), Concat(( Extract( R3.size() - 128 - 1, R3.size() - 192, R3  )  ^ (CONST_BV_S64_V0) ) ,  vfnmadd132_double ( ( Extract( R3.size() - 192 - 1, R3.size() - 256, R3  )  ^ (CONST_BV_S64_V0) ) , Extract( R2.size() - 192 - 1, R2.size() - 256, R2  ) , Extract( R1.size() - 192 - 1, R1.size() - 256, R1  )  ) )) )
PS_R3 = (Concat((CONST_BV_S128_V0), (Concat(((CONST_BV_S64_V0) ^ ((Extract (127, 64, ((R3)))) | (CONST_BV_S64_V0))), ( vfnmadd132_double ( ((CONST_BV_S64_V0) ^ ((Extract (63, 0, ((R3)))) | (CONST_BV_S64_V0))), (Extract (63, 0, ((R2)))), (Extract (63, 0, ((R1))))))))))
