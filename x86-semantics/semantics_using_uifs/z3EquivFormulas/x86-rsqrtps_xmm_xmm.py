...
print('[6;30;44m' + 'Opcode:rsqrtps_xmm_xmm' + '[0m')

R1 = BitVec('R1', 256)
R2 = BitVec('R2', 256)

PK_R2 = (Concat(Extract( R2.size() - 0 - 1, R2.size() - 128, R2  ) , Concat( approx_reciprocal_sqrt_single ( Extract( R1.size() - 128 - 1, R1.size() - 160, R1  )  ) , Concat( approx_reciprocal_sqrt_single ( Extract( R1.size() - 160 - 1, R1.size() - 192, R1  )  ) , Concat( approx_reciprocal_sqrt_single ( Extract( R1.size() - 192 - 1, R1.size() - 224, R1  )  ) ,  approx_reciprocal_sqrt_single ( Extract( R1.size() - 224 - 1, R1.size() - 256, R1  )  ) )))) )
PS_R2 = (Concat((Extract (255, 128, ((R2)))), (Concat(( approx_reciprocal_sqrt_single ( (Extract (127, 96, ((R1)))))), (Concat(( approx_reciprocal_sqrt_single ( (Extract (95, 64, ((R1)))))), (Concat(( approx_reciprocal_sqrt_single ( (Extract (63, 32, ((R1)))))), ( approx_reciprocal_sqrt_single ( (Extract (31, 0, ((R1))))))))))))))
