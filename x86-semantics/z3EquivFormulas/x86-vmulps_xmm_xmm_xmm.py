...
print('[6;30;44m' + 'Opcode:vmulps_xmm_xmm_xmm' + '[0m')
...
PK_R3 = (Concat((CONST_BV_S128_V0), Concat( mul_single ( Extract( R2.size() - 128 - 1, R2.size() - 160, R2  ) , Extract( R1.size() - 128 - 1, R1.size() - 160, R1  )  ) , Concat( mul_single ( Extract( R2.size() - 160 - 1, R2.size() - 192, R2  ) , Extract( R1.size() - 160 - 1, R1.size() - 192, R1  )  ) , Concat( mul_single ( Extract( R2.size() - 192 - 1, R2.size() - 224, R2  ) , Extract( R1.size() - 192 - 1, R1.size() - 224, R1  )  ) ,  mul_single ( Extract( R2.size() - 224 - 1, R2.size() - 256, R2  ) , Extract( R1.size() - 224 - 1, R1.size() - 256, R1  )  ) )))) )
PS_R3 = (Concat((CONST_BV_S32_V0), (Concat((CONST_BV_S32_V0), (Concat((CONST_BV_S32_V0), (Concat((CONST_BV_S32_V0), (Concat(( mul_single ( (Extract (127, 96, ((R2)))), (Extract (127, 96, ((R1)))))), (Concat(( mul_single ( (Extract (95, 64, ((R2)))), (Extract (95, 64, ((R1)))))), (Concat(( mul_single ( (Extract (63, 32, ((R2)))), (Extract (63, 32, ((R1)))))), ( mul_single ( (Extract (31, 0, ((R2)))), (Extract (31, 0, ((R1))))))))))))))))))))
