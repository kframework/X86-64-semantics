...
print('[6;30;44m' + 'Opcode:vhaddpd_ymm_ymm_ymm' + '[0m')

R1 = BitVec('R1', 256)
R2 = BitVec('R2', 256)
R3 = BitVec('R3', 256)

PK_R3 = (Concat(Concat( add_double ( Extract( R1.size() - 64 - 1, R1.size() - 128, R1  ) , Extract( R1.size() - 0 - 1, R1.size() - 64, R1  )  ) ,  add_double ( Extract( R2.size() - 64 - 1, R2.size() - 128, R2  ) , Extract( R2.size() - 0 - 1, R2.size() - 64, R2  )  ) ), Concat( add_double ( Extract( R1.size() - 192 - 1, R1.size() - 256, R1  ) , Extract( R1.size() - 128 - 1, R1.size() - 192, R1  )  ) ,  add_double ( Extract( R2.size() - 192 - 1, R2.size() - 256, R2  ) , Extract( R2.size() - 128 - 1, R2.size() - 192, R2  )  ) )) )
PS_R3 = (Concat((Concat(( add_double ( (Extract (191, 128, ((R1)))), (Extract (255, 192, ((R1)))))), ( add_double ( (Extract (191, 128, ((R2)))), (Extract (255, 192, ((R2)))))))), (Concat(( add_double ( (Extract (63, 0, ((R1)))), (Extract (127, 64, ((R1)))))), ( add_double ( (Extract (63, 0, ((R2)))), (Extract (127, 64, ((R2))))))))))
