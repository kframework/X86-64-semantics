...
print('[6;30;44m' + 'Opcode:btq_r64_r64' + '[0m')
...
PK_CF = ((If ((ULT( ( Extract( R1.size() - 56 - 1, R1.size() - 64, R1  ) & (CONST_BV_S8_V63) ), (CONST_BV_S8_V64) )  ) , ( Extract( ( SignExt(64 - ( R2).size(), ( R2)) >> Concat( BitVecVal(0, SignExt(64 - ( R2).size(), ( R2)).size() - (( Extract( R1.size() - 56 - 1, R1.size() - 64, R1  ) & (CONST_BV_S8_V63) ) ).size()), (( Extract( R1.size() - 56 - 1, R1.size() - 64, R1  ) & (CONST_BV_S8_V63) ) )) ).size() - 63 - 1, ( SignExt(64 - ( R2).size(), ( R2)) >> Concat( BitVecVal(0, SignExt(64 - ( R2).size(), ( R2)).size() - (( Extract( R1.size() - 56 - 1, R1.size() - 64, R1  ) & (CONST_BV_S8_V63) ) ).size()), (( Extract( R1.size() - 56 - 1, R1.size() - 64, R1  ) & (CONST_BV_S8_V63) ) )) ).size() - 64, ( SignExt(64 - ( R2).size(), ( R2)) >> Concat( BitVecVal(0, SignExt(64 - ( R2).size(), ( R2)).size() - (( Extract( R1.size() - 56 - 1, R1.size() - 64, R1  ) & (CONST_BV_S8_V63) ) ).size()), (( Extract( R1.size() - 56 - 1, R1.size() - 64, R1  ) & (CONST_BV_S8_V63) ) )) )  )  ) , ( (If ((( Extract( R2.size() - 0 - 1, R2.size() - 1, R2  ) == (CONST_BV_S1_V0) )  ) , ( (CONST_BV_S1_V0) ) , ( (CONST_BV_S1_V1) ) ))  ) ))  ) == ONE1
PS_CF = (Or((And((True), ((Extract (0, 0, (((Concat((Extract (64, 1, (((Concat((R2), (CONST_BV_S1_V0))) >> (Concat((CONST_BV_S57_V0), ((Extract (7, 0, ((R1)))) & (CONST_BV_S8_V3f)))))))), (CONST_BV_S1_V0))) >> (Concat((CONST_BV_S57_V0), (CONST_BV_S8_V1))))))) == (CONST_BV_S1_V1)))), (False)))
