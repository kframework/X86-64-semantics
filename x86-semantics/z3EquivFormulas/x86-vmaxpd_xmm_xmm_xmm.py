...
print('[6;30;44m' + 'Opcode:vmaxpd_xmm_xmm_xmm' + '[0m')
...
PK_R3 = (Concat((CONST_BV_S128_V0), Concat( (If( maxcmp_double ( Extract( R2.size() - 128 - 1, R2.size() - 192, R2  ) , Extract( R1.size() - 128 - 1, R1.size() - 192, R1  )  ) == ONE1, Extract( R2.size() - 128 - 1, R2.size() - 192, R2  ) , Extract( R1.size() - 128 - 1, R1.size() - 192, R1  ) )),  (If( maxcmp_double ( Extract( R2.size() - 192 - 1, R2.size() - 256, R2  ) , Extract( R1.size() - 192 - 1, R1.size() - 256, R1  )  ) == ONE1, Extract( R2.size() - 192 - 1, R2.size() - 256, R2  ) , Extract( R1.size() - 192 - 1, R1.size() - 256, R1  ) )))) )
PS_R3 = (Concat((CONST_BV_S64_V0), (Concat((CONST_BV_S64_V0), (Concat((If( (( maxcmp_double ( (Extract (127, 64, ((R2)))), (Extract (127, 64, ((R1)))))) == (CONST_BV_S1_V1)),(Extract (127, 64, ((R2)))),(Extract (127, 64, ((R1)))))), (If( (( maxcmp_double ( (Extract (63, 0, ((R2)))), (Extract (63, 0, ((R1)))))) == (CONST_BV_S1_V1)),(Extract (63, 0, ((R2)))),(Extract (63, 0, ((R1))))))))))))
