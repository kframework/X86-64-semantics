...
print('[6;30;44m' + 'Opcode:vfmsubadd132pd_xmm_xmm_xmm' + '[0m')
...
PK_R3 = (Concat((CONST_BV_S128_V0), Concat( vfmsub132_double ( Extract( R3.size() - 128 - 1, R3.size() - 192, R3  ) , Extract( R2.size() - 128 - 1, R2.size() - 192, R2  ) , Extract( R1.size() - 128 - 1, R1.size() - 192, R1  )  ) ,  vfmsub132_double ( Extract( R3.size() - 192 - 1, R3.size() - 256, R3  ) , vfnmsub132_double ( Extract( R2.size() - 192 - 1, R2.size() - 256, R2  ) , Extract( R2.size() - 192 - 1, R2.size() - 256, R2  ) , Concat((CONST_BV_S32_V0), Extract( R2.size() - 128 - 1, R2.size() - 160, R2  ) ) ), Extract( R1.size() - 192 - 1, R1.size() - 256, R1  )  ) )) )
PS_R3 = (Concat((CONST_BV_S128_V0), (Concat(( vfmsub132_double ( (Extract (127, 64, ((R3)))), (Extract (127, 64, ((R2)))), (Extract (127, 64, ((R1)))))), ( vfmsub132_double ( (Extract (63, 0, ((R3)))), ( vfnmsub132_double ( (Extract (63, 0, ((R2)))), (Extract (63, 0, ((R2)))), (Concat((CONST_BV_S32_V0), (Extract (127, 96, ((R2)))))))), (Extract (63, 0, ((R1))))))))))
