...
print('[6;30;44m' + 'Opcode:vsqrtps_xmm_xmm' + '[0m')
...
PK_R2 = (Concat((CONST_BV_S128_V0), extractMask(Concat( sqrt_single ( (CONST_BV_S32_V0) ) , Concat( sqrt_single ( (CONST_BV_S32_V0) ) , Concat( sqrt_single ( (CONST_BV_S32_V0) ) , Concat( sqrt_single ( (CONST_BV_S32_V0) ) , Concat( sqrt_single ( Extract( R1.size() - 128 - 1, R1.size() - 160, R1  )  ) , Concat( sqrt_single ( Extract( R1.size() - 160 - 1, R1.size() - 192, R1  )  ) , Concat( sqrt_single ( Extract( R1.size() - 192 - 1, R1.size() - 224, R1  )  ) ,  sqrt_single ( Extract( R1.size() - 224 - 1, R1.size() - 256, R1  )  ) ))))))), 128, 0)) )
PS_R2 = (Concat((CONST_BV_S128_V0), (Concat(( sqrt_single ( (Extract (127, 96, ((R1)))))), (Concat(( sqrt_single ( (Extract (95, 64, ((R1)))))), (Concat(( sqrt_single ( (Extract (63, 32, ((R1)))))), ( sqrt_single ( (Extract (31, 0, ((R1))))))))))))))
