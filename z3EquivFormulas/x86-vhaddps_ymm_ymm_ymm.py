...
print('[6;30;44m' + 'Opcode:vhaddps_ymm_ymm_ymm' + '[0m')

R1 = BitVec('R1', 256)
R2 = BitVec('R2', 256)
R3 = BitVec('R3', 256)

PK_R3 = (Concat(Concat(Concat( add_single ( Extract( R1.size() - 32 - 1, R1.size() - 64, R1  ) , Extract( R1.size() - 0 - 1, R1.size() - 32, R1  )  ) ,  add_single ( Extract( R1.size() - 96 - 1, R1.size() - 128, R1  ) , Extract( R1.size() - 64 - 1, R1.size() - 96, R1  )  ) ), Concat( add_single ( Extract( R2.size() - 32 - 1, R2.size() - 64, R2  ) , Extract( R2.size() - 0 - 1, R2.size() - 32, R2  )  ) ,  add_single ( Extract( R2.size() - 96 - 1, R2.size() - 128, R2  ) , Extract( R2.size() - 64 - 1, R2.size() - 96, R2  )  ) )), Concat(Concat( add_single ( Extract( R1.size() - 160 - 1, R1.size() - 192, R1  ) , Extract( R1.size() - 128 - 1, R1.size() - 160, R1  )  ) ,  add_single ( Extract( R1.size() - 224 - 1, R1.size() - 256, R1  ) , Extract( R1.size() - 192 - 1, R1.size() - 224, R1  )  ) ), Concat( add_single ( Extract( R2.size() - 160 - 1, R2.size() - 192, R2  ) , Extract( R2.size() - 128 - 1, R2.size() - 160, R2  )  ) ,  add_single ( Extract( R2.size() - 224 - 1, R2.size() - 256, R2  ) , Extract( R2.size() - 192 - 1, R2.size() - 224, R2  )  ) ))) )
PS_R3 = (Concat((Concat((Concat((Concat(( add_single ( (Extract (223, 192, ((R1)))), (Extract (255, 224, ((R1)))))), ( add_single ( (Extract (159, 128, ((R1)))), (Extract (191, 160, ((R1)))))))), ( add_single ( (Extract (223, 192, ((R2)))), (Extract (255, 224, ((R2)))))))), ( add_single ( (Extract (159, 128, ((R2)))), (Extract (191, 160, ((R2)))))))), (Concat((Concat((Concat(( add_single ( (Extract (95, 64, ((R1)))), (Extract (127, 96, ((R1)))))), ( add_single ( (Extract (31, 0, ((R1)))), (Extract (63, 32, ((R1)))))))), ( add_single ( (Extract (95, 64, ((R2)))), (Extract (127, 96, ((R2)))))))), ( add_single ( (Extract (31, 0, ((R2)))), (Extract (63, 32, ((R2))))))))))
