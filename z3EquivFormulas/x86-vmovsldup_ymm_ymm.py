...
print('[6;30;44m' + 'Opcode:vmovsldup_ymm_ymm' + '[0m')

R1 = BitVec('R1', 256)
R2 = BitVec('R2', 256)

PK_R2 = (Concat(Concat(Concat(Extract( R1.size() - 32 - 1, R1.size() - 64, R1  ) , Extract( R1.size() - 32 - 1, R1.size() - 64, R1  ) ), Concat(Extract( R1.size() - 96 - 1, R1.size() - 128, R1  ) , Extract( R1.size() - 96 - 1, R1.size() - 128, R1  ) )), Concat(Concat(Extract( R1.size() - 160 - 1, R1.size() - 192, R1  ) , Extract( R1.size() - 160 - 1, R1.size() - 192, R1  ) ), Concat(Extract( R1.size() - 224 - 1, R1.size() - 256, R1  ) , Extract( R1.size() - 224 - 1, R1.size() - 256, R1  ) ))) )
PS_R2 = (Concat((Concat((Concat((Extract (223, 192, ((R1)))), (Extract (223, 192, ((R1)))))), (Concat((Extract (159, 128, ((R1)))), (Extract (159, 128, ((R1)))))))), (Concat((Concat((Extract (95, 64, ((R1)))), (Extract (95, 64, ((R1)))))), (Concat((Extract (31, 0, ((R1)))), (Extract (31, 0, ((R1))))))))))
