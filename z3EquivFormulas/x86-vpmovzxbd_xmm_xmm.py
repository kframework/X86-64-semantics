...
print('[6;30;44m' + 'Opcode:vpmovzxbd_xmm_xmm' + '[0m')
...
PK_R2 = (Concat((CONST_BV_S128_V0), Concat(Concat(Concat((CONST_BV_S24_V0), Extract( R1.size() - 224 - 1, R1.size() - 232, R1  ) ), Concat((CONST_BV_S24_V0), Extract( R1.size() - 232 - 1, R1.size() - 240, R1  ) )), Concat(Concat((CONST_BV_S24_V0), Extract( R1.size() - 240 - 1, R1.size() - 248, R1  ) ), Concat((CONST_BV_S24_V0), Extract( R1.size() - 248 - 1, R1.size() - 256, R1  ) )))) )
PS_R2 = (Concat((CONST_BV_S128_V0), (Concat((Concat((Concat((CONST_BV_S24_V0), (Extract (31, 24, ((R1)))))), (Concat((CONST_BV_S24_V0), (Extract (23, 16, ((R1)))))))), (Concat((Concat((CONST_BV_S24_V0), (Extract (15, 8, ((R1)))))), (Concat((CONST_BV_S24_V0), (Extract (7, 0, ((R1))))))))))))
