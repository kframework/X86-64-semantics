...
print('[6;30;44m' + 'Opcode:vpmovzxwd_xmm_xmm' + '[0m')
...
PK_R2 = (Concat((CONST_BV_S128_V0), Concat(Concat(Concat((CONST_BV_S16_V0), Extract( R1.size() - 192 - 1, R1.size() - 208, R1  ) ), Concat((CONST_BV_S16_V0), Extract( R1.size() - 208 - 1, R1.size() - 224, R1  ) )), Concat(Concat((CONST_BV_S16_V0), Extract( R1.size() - 224 - 1, R1.size() - 240, R1  ) ), Concat((CONST_BV_S16_V0), Extract( R1.size() - 240 - 1, R1.size() - 256, R1  ) )))) )
PS_R2 = (Concat((CONST_BV_S128_V0), (Concat((Concat((Concat((CONST_BV_S16_V0), (Extract (63, 48, ((R1)))))), (Concat((CONST_BV_S16_V0), (Extract (47, 32, ((R1)))))))), (Concat((Concat((CONST_BV_S16_V0), (Extract (31, 16, ((R1)))))), (Concat((CONST_BV_S16_V0), (Extract (15, 0, ((R1))))))))))))
