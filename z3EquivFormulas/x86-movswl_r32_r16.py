...
print('[6;30;44m' + 'Opcode:movswl_r32_r16' + '[0m')

R1 = BitVec('R1', 64)
R2 = BitVec('R2', 64)
CONST_BV_S32_V0 = BitVecVal(0, 32)

PK_R2 = (Concat((CONST_BV_S32_V0), Extract( SignExt(64 - ( Extract( R1.size() - 48 - 1, R1.size() - 64, R1  )).size(), ( Extract( R1.size() - 48 - 1, R1.size() - 64, R1  ))).size() - 32 - 1, SignExt(64 - ( Extract( R1.size() - 48 - 1, R1.size() - 64, R1  )).size(), ( Extract( R1.size() - 48 - 1, R1.size() - 64, R1  ))).size() - 64, SignExt(64 - ( Extract( R1.size() - 48 - 1, R1.size() - 64, R1  )).size(), ( Extract( R1.size() - 48 - 1, R1.size() - 64, R1  )))  ) ) )
PS_R2 = (Concat((CONST_BV_S32_V0), (Extract (31, 0, ((SignExt( 64 - (Extract (15, 0, ((R1)))).size(), (Extract (15, 0, ((R1)))))))))))
