...
print('[6;30;44m' + 'Opcode:vpmovzxbd_ymm_xmm' + '[0m')

R1 = BitVec('R1', 256)
R2 = BitVec('R2', 256)
CONST_BV_S24_V0 = BitVecVal(0, 24)

PK_R2 = (Concat(Concat(Concat(Concat((CONST_BV_S24_V0), Extract( R1.size() - 192 - 1, R1.size() - 200, R1  ) ), Concat((CONST_BV_S24_V0), Extract( R1.size() - 200 - 1, R1.size() - 208, R1  ) )), Concat(Concat((CONST_BV_S24_V0), Extract( R1.size() - 208 - 1, R1.size() - 216, R1  ) ), Concat((CONST_BV_S24_V0), Extract( R1.size() - 216 - 1, R1.size() - 224, R1  ) ))), Concat(Concat(Concat((CONST_BV_S24_V0), Extract( R1.size() - 224 - 1, R1.size() - 232, R1  ) ), Concat((CONST_BV_S24_V0), Extract( R1.size() - 232 - 1, R1.size() - 240, R1  ) )), Concat(Concat((CONST_BV_S24_V0), Extract( R1.size() - 240 - 1, R1.size() - 248, R1  ) ), Concat((CONST_BV_S24_V0), Extract( R1.size() - 248 - 1, R1.size() - 256, R1  ) )))) )
PS_R2 = (Concat((Concat((Concat((Concat((CONST_BV_S24_V0), (Extract (63, 56, ((R1)))))), (Concat((CONST_BV_S24_V0), (Extract (55, 48, ((R1)))))))), (Concat((Concat((CONST_BV_S24_V0), (Extract (47, 40, ((R1)))))), (Concat((CONST_BV_S24_V0), (Extract (39, 32, ((R1)))))))))), (Concat((Concat((Concat((CONST_BV_S24_V0), (Extract (31, 24, ((R1)))))), (Concat((CONST_BV_S24_V0), (Extract (23, 16, ((R1)))))))), (Concat((Concat((CONST_BV_S24_V0), (Extract (15, 8, ((R1)))))), (Concat((CONST_BV_S24_V0), (Extract (7, 0, ((R1))))))))))))
