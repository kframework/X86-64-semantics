...
print('[6;30;44m' + 'Opcode:maxsd_xmm_xmm' + '[0m')

R1 = BitVec('R1', 256)
R2 = BitVec('R2', 256)
CONST_BV_S1_V1 = BitVecVal(0x1, 1)

PK_R2 = (Concat(Extract( R2.size() - 0 - 1, R2.size() - 192, R2  ) ,  (If( maxcmp_double ( Extract( R2.size() - 192 - 1, R2.size() - 256, R2  ) , Extract( R1.size() - 192 - 1, R1.size() - 256, R1  )  ) == ONE1, Extract( R2.size() - 192 - 1, R2.size() - 256, R2  ) , Extract( R1.size() - 192 - 1, R1.size() - 256, R1  ) ))) )
PS_R2 = (Concat((Extract (255, 128, ((R2)))), (Concat((Extract (127, 64, ((R2)))), (If( (( maxcmp_double ( (Extract (63, 0, ((R2)))), (Extract (63, 0, ((R1)))))) == (CONST_BV_S1_V1)),(Extract (63, 0, ((R2)))),(Extract (63, 0, ((R1))))))))))
