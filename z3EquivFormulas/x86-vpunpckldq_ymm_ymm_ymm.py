...
print('[6;30;44m' + 'Opcode:vpunpckldq_ymm_ymm_ymm' + '[0m')

R1 = BitVec('R1', 256)
R2 = BitVec('R2', 256)
R3 = BitVec('R3', 256)

PK_R3 = (Concat(Concat(Concat(Extract( R1.size() - 64 - 1, R1.size() - 96, R1  ) , Extract( R2.size() - 64 - 1, R2.size() - 96, R2  ) ), Concat(Extract( R1.size() - 96 - 1, R1.size() - 128, R1  ) , Extract( R2.size() - 96 - 1, R2.size() - 128, R2  ) )), Concat(Concat(Extract( R1.size() - 192 - 1, R1.size() - 224, R1  ) , Extract( R2.size() - 192 - 1, R2.size() - 224, R2  ) ), Concat(Extract( R1.size() - 224 - 1, R1.size() - 256, R1  ) , Extract( R2.size() - 224 - 1, R2.size() - 256, R2  ) ))) )
PS_R3 = (Concat((Concat((Concat((Extract (191, 160, ((R1)))), (Extract (191, 160, ((R2)))))), (Concat((Extract (159, 128, ((R1)))), (Extract (159, 128, ((R2)))))))), (Concat((Concat((Extract (63, 32, ((R1)))), (Extract (63, 32, ((R2)))))), (Concat((Extract (31, 0, ((R1)))), (Extract (31, 0, ((R2))))))))))
